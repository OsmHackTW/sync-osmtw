...
'''
This script is Python 3 version of polyshp2osm.py.
To trace the original version, just see:

  https://svn.openstreetmap.org/applications/utils/import/shp2osm/polyshp2osm.py
'''
...
def start_new_file():
    ...
    if open_file:
        close_file()
    open_file = open("%s" % (file_name), "w")
    open_file.write("<osm version='0.6'>")
...
def add_ring_nodes(ring):
    ...
    if range(ring.GetPointCount() - 1) == 0 or ring.GetPointCount() == 0:
        sys.stderr.write("Degenerate ring.\n")
        return
    for count in range(ring.GetPointCount() - 1):
        ids.append(id_counter)
        open_file.write("<node timestamp='1969-12-31T23:59:59Z' changeset='-1' id='%s' version='1' lon='%s' lat='%s' />" % (id_counter, ring.GetX(count), ring.GetY(count)))
        id_counter += 1
...
def add_ring_way(ring):
    ...
    for count in range(ring.GetPointCount() - 1):
        ids.append(id_counter)
        open_file.write("<node timestamp='1969-12-31T23:59:59Z' changeset='-1' version='1' id='%s' lon='%s' lat='%s' />" % (id_counter, ring.GetX(count), ring.GetY(count)))
        id_counter += 1
    if len(ids) == 0:
        return None
    open_file.write("<way timestamp='1969-12-31T23:59:59Z' changeset='-1' id='%s' version='1'>" % id_counter)
    way_id = id_counter
    id_counter += 1
    count = 0
    for i in ids:
        open_file.write("<nd ref='%s' />" % i)
        count += 1
        if count >= Max_Waylength - 1:
            ...
            id_counter += 1
            open_file.write("<nd ref='%s' />" % i)
    open_file.write("<nd ref='%s' />" % ids[0])
    open_file.write("</way>")
    ringways.append(way_id)
...
#Write out the fields for the way
                    ...
                    for field in range(field_count):
                        ...
                        if namespace and name and value and name not in boring_tags:
                            open_file.write("<tag k='%s:%s' v='%s' />" % (namespace, name, clean_attr(value)))
...
#Perform the specified field mappting
                    ...
                    for key, value in tags.items():
                        if key and value:
                            open_file.write("<tag k='%s' v='%s' />" % (key, clean_attr(value)))
#Write fixed tabs
                    for name, value in fixed_tags.items():
                        open_file.write("<tag k='%s' v='%s' />" % (name, clean_attr(value)))
                    open_file.write("</way>")
...
#add the inner ways
                ...
                open_file.write("<relation timestamp='1969-12-31T23:59:59Z' changeset='-1' id='%s' version='1' ><tag k='type' v='multipolygon' />" % id_counter)
                id_counter += 1
                for way in outerways:
                    open_file.write('<member type="way" ref="%s" role="outer" />' % way)
                for way in innerways:
                    open_file.write('<member type="way" ref="%s" role="inner" />' % way)
